...
import { OhImg } from "@ohimg/ohimg-js";
    ...
    async postBuild({ outDir, siteConfig, plugins = [] }) {
      ...
      const processHtmlFile = async (filePath, route) => {
        try {
          ...
          $("head").append(
            `<meta property="og:image" content="${ogImgUrl}" data-rh="true">`
          );
          $("head").append(
            `<meta name="twitter:image" content="${ogImgUrl}" data-rh="true">`
          );
          $("head").append(
            `<meta name="image" content="${ogImgUrl}" data-rh="true">`
          );
      ...
      for (const route of processableRoutes) {
        ...
        const directFilePath = path.join(
          outDir,
          route.path.replace(/\/$/, "").replace(/\.html$/, "") + ".html"
        );
